        ...
        public void OnLoad() {
            for (int index = 0; index < SaveData.Count; index++) {
                ...
                if (RandomUtils.GetTime() < consumeable.EndTime) {
                    UseConsumable((Consumable) Configs.Instance.Items.First(it => it.Id == consumeable.Id));
                }
            }
        ...
        public void Update() {
            for (int index = 0; index < SaveData.Count; index++) {
                ...
                if (RandomUtils.GetTime() > consumeable.EndTime) {
                    ...
                    index--;
                    FinishConsumable((Consumable) Configs.Instance.Items.First(it => it.Id == consumeable.Id));
                    ConsumableUi.Instance.Refresh();
                }
            }
        }
